...
programSettings::programSettings(QWidget *parent)
    : QWidget(parent)
    , m_pTopLabel(new DLabel(tr("程序配置")))
    , m_pCentralLabel(new DLabel(tr("请选择程序")))
    , m_pButton(new DPushButton(tr("下一步")))
    , m_pCommandEraseButton(new DCommandLinkButton(tr("全部清除")))
    , m_pCommandChooseButton(new DCommandLinkButton(tr("选择程序deb包")))
    , m_pListView(new DListViewMouse)
    , m_pStandardItemModel(new QStandardItemModel)
    , clearButton (new DIconButton(nullptr))
    , MainLayout(new QVBoxLayout(this))
{

    //clearButton->setIcon(QIcon(":/images/清除1.svg"));
    ...
    connect(m_pCommandChooseButton,&DCommandLinkButton::clicked,this,&programSettings::chooseButtonSlots,Qt::QueuedConnection);
    connect(m_pCommandEraseButton,&DCommandLinkButton::clicked,this,&programSettings::eraseButtonSlots,Qt::QueuedConnection);
    connect(m_pButton,&DPushButton::clicked,this,&programSettings::clikedButton,Qt::QueuedConnection);
}
...
void programSettings::chooseButtonSlots()
{
    ...
    pFileDialog->setNameFilter(tr("*.deb"));
    pFileDialog->show();
    pFileDialog->exec();
...
void programSettings::eraseButtonSlots()
{
...
void programSettings::clikedButton()
{
    emit clikedNext();
}

DListViewMouse::DListViewMouse(QWidget *parent)
    : DListView(parent)
{
    this->setMouseTracking(true);
    this->setEditTriggers(QAbstractItemView::NoEditTriggers);
    this->setState(QAbstractItemView::NoState);
    this->setStyleSheet("QListWidget::item::hover{background-color:rgba(0,0,0,0.3)}");
}

void DListViewMouse::currentChanged(const QModelIndex &current, const QModelIndex &previous)
{
    emit currentChange(current, previous);
}

void DListViewMouse::mouseMoveEvent(QMouseEvent *e)
{
    ;
}
